                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="produkTable">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <select name="produk_id[]" class="form-control produk-select" onchange="updateharga_jual(this)" required>
                                        <option value="" disabled selected>Pilih Produk</option>
                                        @foreach ($produks as $produk)
                                            <option value="{{ $produk->id }}" data-harga_jual="{{ $produk->harga_jual }}">
                                                {{ $produk->nama }}
                                            </option>
                                        @endforeach
                                    </select>
...
<script>
    function updateharga_jual(select) {
        let harga_jual = select.options[select.selectedIndex].getAttribute("data-harga_jual") || 0;
        let row = select.closest("tr");
        row.querySelector(".harga_jual").value = "Rp" + parseInt(harga_jual).toLocaleString();
        calculateTotal(row.querySelector(".quantity"));
    }

    function calculateTotal(input) {
        let row = input.closest("tr");
        let harga_jual = parseInt(row.querySelector(".produk-select").selectedOptions[0].getAttribute("data-harga_jual")) || 0;
